-- ===================================================
-- UPDATE LEVEL REQUIREMENTS - NEW XP SYSTEM
-- ===================================================
...
BEGIN
  ...
  LOOP
    IF total_xp < (cumulative_xp + level_record.xp_required) THEN
      -- Found the current level
      current_level := level_record.level;
      
      -- Calculate current level XP (XP within this level)
      current_level_xp := total_xp - cumulative_xp;
      
      -- Calculate XP needed to reach next level
      xp_to_next := level_record.xp_required - current_level_xp;
      
      RETURN QUERY SELECT current_level, current_level_xp, xp_to_next;
      RETURN;
    END IF;
    
    -- Add this level's requirement to cumulative
    cumulative_xp := cumulative_xp + level_record.xp_required;
    current_level := level_record.level + 1;
  END LOOP;
  ...
  RETURN QUERY SELECT current_level, total_xp - cumulative_xp + (SELECT lr.xp_required FROM public.level_rewards lr WHERE lr.level = current_level), 0;
END;
$$;
...
DECLARE
    ...
    level_calc RECORD;
BEGIN
    FOR user_record IN SELECT uls.user_id as id, uls.lifetime_xp FROM public.user_level_stats uls WHERE uls.lifetime_xp > 0 LOOP
