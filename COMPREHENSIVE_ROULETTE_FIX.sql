-- COMPREHENSIVE ROULETTE FIX
-- This script fixes the 500 errors in roulette-engine and 404 errors in ensure_user_level_stats
...
-- 8. Clear any existing problematic roulette rounds
DELETE FROM public.roulette_rounds 
WHERE status IN ('betting', 'spinning') 
AND (daily_seed_id IS NULL OR nonce_id IS NULL);

-- 9. Create a new test round with proper daily seed
...
SELECT 
    'betting',
    (NOW() + INTERVAL '25 seconds')::timestamp with time zone,
    (NOW() + INTERVAL '29 seconds')::timestamp with time zone,
    ds.id,
    1,
    ds.server_seed,
    ds.server_seed_hash,
    1
FROM public.daily_seeds ds
WHERE ds.date = CURRENT_DATE
AND NOT EXISTS (
    SELECT 1 FROM public.roulette_rounds 
    WHERE status IN ('betting', 'spinning')
);

-- 10. Verify the function works
SELECT public.ensure_user_level_stats('00000000-0000-0000-0000-000000000000'::uuid) as test_result;

-- 11. Show current roulette rounds status
SELECT 
    id,
    status,
    betting_end_time,
