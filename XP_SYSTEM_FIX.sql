...
BEGIN
  RAISE NOTICE 'Starting XP migration for all users...';
  
  -- For each user, recalculate XP based on total_wagered
  FOR user_record IN 
    SELECT id, total_wagered, current_level, lifetime_xp, username
  ...
  LOOP
    ...
    RAISE NOTICE 'Updated user % (%) - Total wagered: %, New XP: %, New Level: %', 
      user_record.username, user_record.id, user_record.total_wagered, correct_lifetime_xp, correct_level_data.level;
  END LOOP;
...
-- Step 8: Test the new XP calculation
SELECT 
  'Testing XP calculation:' as test,
  public.calculate_xp_from_bet(0.10) as xp_from_10_cents,
  public.calculate_xp_from_bet(1.00) as xp_from_1_dollar,
  public.calculate_xp_from_bet(10.00) as xp_from_10_dollars,
  public.calculate_xp_from_bet(100.00) as xp_from_100_dollars;