-- Simple single-table diagnostics that shows everything at once
-- This returns all diagnostic info in one easy-to-read table
...
BEGIN
  ...
  -- Test 6: Check recent roulette activity
  ...
  WHERE user_id = test_user_id AND created_at > NOW() - INTERVAL '1 hour';
  
  SELECT COUNT(*) INTO game_history_count
  FROM public.game_history
  WHERE user_id = test_user_id AND game_type = 'roulette' AND created_at > NOW() - INTERVAL '1 hour';
